...
gcloud access-context-manager perimeters describe ai_core \
  --policy=829925107376 --format=json > przed-przejeciem.json
```
...
```bash
./tools/brownfield_import.sh --policy-id 829925107376 --perimeter ai_core
```
...
```bash
./tools/brownfield_import.sh --policy-id 829925107376 --perimeter ai_core --write-import
```
...
```bash
gcloud access-context-manager perimeters describe ai_core \
  --policy=829925107376 --format=json > po-przejeciu.json
...
gcloud logging read \
  'logName:"cloudaudit.googleapis.com%2Factivity"
   AND protoPayload.resourceName:"servicePerimeters/ai_core"' \
  --organization=179248107504 --freshness=1h \
...
```bash
gcloud access-context-manager perimeters describe ai_core --policy=829925107376 --format=json \
| python3 -c "
...
gcloud access-context-manager perimeters dry-run update ai_core \
  --policy=829925107376 --set-ingress-policies=bez-sieroty.yaml
```
...
```bash
export USER_PROJECT_OVERRIDE=true GOOGLE_BILLING_PROJECT=rwlab-vpcsc-adm-46bc
```
