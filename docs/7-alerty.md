...
gcloud logging read 'protoPayload.metadata."@type"="type.googleapis.com/google.cloud.audit.VpcServiceControlAuditMetadata"' \
  --project=rwlab-vpcsc-adm-46bc --bucket=vpcsc-violations --location=eu --view=vpcsc-violations \
...
```bash
python3 tools/kanaly_check.py --project rwlab-vpcsc-adm-46bc
```
...
3. **Potwierdź maszynowo tor Pub/Sub** — wiadomość niesie pełny obiekt incydentu:
   ```bash
   gcloud pubsub subscriptions pull vpcsc-alerts-ewidencja --project=rwlab-vpcsc-adm-46bc --limit=20 --format=json \
