...
2. **`gcloud` potrzebuje projektu rozliczeniowego dla tego API.** Wywołania ACM kontem użytkownika wymagają
   ...
   ```bash
   gcloud config set billing/quota_project rwlab-vpcsc-adm-46bc
   gcloud access-context-manager policies list --organization 179248107504
   ```
...
for R in resourcemanager.organizationViewer accesscontextmanager.policyReader browser \
  ...
  gcloud organizations add-iam-policy-binding 179248107504 \
...
```
rola: organizations/179248107504/roles/vpcScPerimeterWriter
...
gcloud access-context-manager policies create \
  --organization 179248107504 --scopes=folders/578145433296 --title "VPC-SC sandbox"

# i deleguje nam admina na TEJ JEDNEJ polityce (nie na organizacji)
gcloud access-context-manager policies add-iam-policy-binding 829925107376 \
  --member=serviceAccount:sa-vpcsc-apply@rwlab-vpcsc-adm-46bc.iam.gserviceaccount.com \
