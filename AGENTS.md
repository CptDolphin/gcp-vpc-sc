...
| **Werdykt bramki jest odróżnialny od awarii bramki bez czytania logu** | krok `werdykt bramek tresci` (`if: always()`) w `.github/actions/bramki-tresci` — klasyfikuje z DWÓCH znaczników pisanych przez kroki, które naprawdę się wykonały; oba tory (PR i apply) | zmierzone na tym repozytorium: krok instalujący `conftest` padł na pobraniu, WSZYSTKIE bramki poniżej dostały `outcome=skipped`, a check wyglądał identycznie jak odrzucenie wniosku przez regułę. Kolor przebiegu nie niesie werdyktu — nośnikiem jest adnotacja (`::error title=…`), bo kroków WEWNĄTRZ akcji złożonej REST API nie wystawia w ogóle. Anty-tautologia mieszka w selfteście startera; tutaj pilnuje tego sam krok werdyktu (DEC-26) |
| Narzędzia bramek mają **jedno źródło, pin, sumę kontrolną i ponawianie** | `.github/actions/narzedzia` — wołana przez `bramki-tresci`, `plan`, `apply`, `intake`, `external-intake` i `publish-gates` | ten sam `curl` stał w SZEŚCIU plikach, w żadnym z weryfikacją pobranego pliku binarnego — na ścieżce jedynego mutatora granicy tej organizacji. `--retry-all-errors` ponawia także błąd 60, czyli dokładnie zmierzony tryb awarii; `sha256sum -c` stoi PRZED `mv` na `PATH` (DEC-26) |
| `continue-on-error` ma **zapisany powód i widoczny `outcome`**, a na powierzchni bramek jest zakazane | `tools/continue_on_error_check.py` w `.github/actions/bramki-tresci` (oba tory) | zmierzone: krok z tą flagą raportuje w REST API `steps[].conclusion: "success"` MIMO `##[error]` w logu — API nie wystawia per-krokowego `outcome`. Krok, którego porażki nie widać w API, jest gorszy niż krok bez flagi (DEC-26) |
